...
class Arm3DDataset(Dataset):
    def __init__(
        ...
        if self.split == 'evaluation': self.split = 'val'
    ...
    def __getitem__(self, index):
        ...
        if hand_type is None:
            ...
            for hand_type in ['left', 'right']:
                ...
                if hand_params[hand_type]['visible'] == 0: continue
        ...
        visible_hand = hand_param['visible']
        valid_hand_param = hand_param['valid_param']
        valid_hand_j3d = hand_param['valid_j3d']
        ...
        if 'occluded_jnt' in hand_param:
            occluded_hand_jnt = hand_param['occluded_jnt']
        else:
            occluded_hand_jnt = np.zeros(21)

        hand_type = 0 if hand_type == 'left' else 1

        visible_arm = arm_param['visible']

        valid_arm_param = arm_param['valid_param'] 
        valid_arm_j3d = arm_param['valid_j3d']
        ...
        valid_arm = (arm_j3d != 0).any()
        valid_arm_param = np.logical_and(valid_arm_param, valid_arm)
        valid_arm_j3d = np.logical_and(valid_arm_j3d, valid_arm)
        ...
        arm_sparse_kpe = self.create_sparse_kpe(camera_model, arm_bbox, org_img_size)
        ...
        try:
            ...
            hand_crop = np.zeros((*self.image_size, 3))
            hand_mask_crop = np.zeros(self.image_size)
            hand_j2d = np.zeros((21, 2))
            hand_crop_size = (0, 0)
            visible_hand = 0
            K_hand = camera_model.get_K()
        ...
        hand_hms, hand_vis_2d = self.generate_hms2d(hand_j2d, visible_hand, num_joints=21)
        arm_hms, arm_vis_2d = self.generate_hms2d(arm_j2d, visible_arm, num_joints=3)
        ...
        occluded_hand_jnt = torch.tensor(occluded_hand_jnt).long()
        ...
        valid_hand_param = torch.tensor(valid_hand_param).float()
        valid_hand_j3d = torch.tensor(valid_hand_j3d).float()
        
        visible_arm = torch.tensor(visible_arm).float()
        valid_arm_param = torch.tensor(valid_arm_param).float()
        valid_arm_j3d = torch.tensor(valid_arm_j3d).float()
        ...
        if self.no_arm:
            arm_crop.fill_(0)
            arm_mask_crop.fill_(0)
            arm_sparse_kpe.fill_(0)

        if self.no_kpe:
            hand_sparse_kpe.fill_(0)
            arm_sparse_kpe.fill_(0)
        ...
        data = {
            ...
            'valid_hand_param': valid_hand_param,
            'valid_hand_j3d': valid_hand_j3d,
            'occluded_hand_jnt': occluded_hand_jnt,

            'visible_arm': visible_arm,
            'valid_arm_param': valid_arm_param,
            'valid_arm_j3d': valid_arm_j3d,
    ...
    def generate_d_hms_target(self, d3d, joints_vis, heatmap_length=300, sigma=3.0):
        '''
        Generates 1D Gaussian heatmaps for the distance of joints.
        '''
        d3d      = np.asarray(d3d,        dtype=np.float32).reshape(-1)
        joints_vis = np.asarray(joints_vis, dtype=np.float32).reshape(-1)
        J        = d3d.shape[0]
        L        = heatmap_length
        sigma_sq2 = 2.0 * sigma ** 2

        mu = np.rint(d3d).astype(np.int32)          # (J,)
        valid = (joints_vis >= 0.5) & (mu >= 0) & (mu < L)

        z_grid = np.arange(L, dtype=np.float32)     # (L,)
        delta  = z_grid[None, :] - mu[:, None]      # (J, L)
        target = np.exp(- (delta ** 2) / sigma_sq2, dtype=np.float32)

        # Mask out invalid joints
        target[~valid, :] = 0.0
        weight = valid.astype(np.float32)[:, None]  # (J,1)
