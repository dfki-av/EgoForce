...
from camera_models import KannalaBrandtK3CameraModel 
...
class ReInterHandEgoLoader(Dataset):
    IMG_WIDTH = 1024
    IMG_HEIGHT = 1024
    ...
    def __init__(self, data_root, split, get_camera=False, filter_only_hands=True, **kwargs) -> None:
        print(f'Loading ReInterHandEgo Dataset from: {data_root}')
        self.data_root = data_root
        self.split = split
        ...
        self.annotations = h5py.File(f'{self.anno_root}/egocam_hand_arm_annotations_v1.h5', "r")[split]
        
        self.load_tar_index_map(self.data_root)

        self.sample_loader = AsyncSamplerLoader(self.data_root)
        
        self.sample_keys = list(self.tar_index_map['egocam'].keys())

        # cam_params

        with open(f'{self.anno_root}/filtered_hand_hand_non_overlap_indices.json', 'r') as f:
    ...
    def get_image(self, seq, sample_name):
        rgb_key = f'{seq}.{sample_name}.egocam.png'
        rgb_info = self.tar_index_map['egocam'][rgb_key]        
    ...
    def __getitem__(self, index):
        ...
        sequence_name, sample_name, component_name, ext = sample_name.split('.')
        ...
        rgb = self.get_image(sequence_name, sample_name)
        ...
        data['camera_params']['camera_type'] = 4
        data['extras'] = dict()
        data['extras']['rgb_np'] = rgb
        data['extras']['hand_hand_overlap'] = anno_data["hand_hand_overlap"]
        ...
        data['extras']['dataset'] = 'reinterhandego'
        ...
        if self.get_camera:
            ...
            projection_params = camera_params['projection_params'][:4]
            camera_model = KannalaBrandtK3CameraModel(focal_length, principal_point, projection_params, width, height)
    ...
    def get_annotation(self, index):
        ...
        return {
            ...
            'hand_hand_overlap': data["hand_hand_overlap"],
