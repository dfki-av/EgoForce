...
class UnZipperIterDataPipe(IterDataPipe[T]):
    ...
    def __new__(cls,
        ...
        if len(instance_ids) == 0:
            raise RuntimeError("All instances are being filtered out in UnZipperIterDataPipe. Please check"
        ...
        container = _UnZipperIterDataPipe(source_datapipe, instance_ids, buffer_size)
...
class _UnZipperIterDataPipe(_ForkerIterDataPipe):
    ...
    def get_next_element_by_instance(self, instance_id: int):
        r"""
        Note: Each element returned from the source datapipe is required to be a sequence that can
