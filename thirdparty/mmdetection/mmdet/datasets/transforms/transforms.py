...
@TRANSFORMS.register_module()
class RandomCropWithKeypoints(RandomCrop):
    ...
    def _crop_data(self, results: dict, crop_size: Tuple[int, int],
        ...
        # Ensure that the crop size does not exceed the image size
        if crop_h > img_h or crop_w > img_w:
            # Adjust the crop size to fit the image dimensions
        ...
        if keypoints is not None:
            ...
            if visible_keypoints.size > 0:
                # x and y coordinates of visible keypoints
                kp_x = visible_keypoints[..., 0]
                kp_y = visible_keypoints[..., 1]

                # Compute valid ranges for the top-left corner (x1, y1) of the crop
                x1_min = max(0, int(np.floor(kp_x.min() - (crop_w - 1))))
                x1_max = min(int(np.floor(kp_x.max())), img_w - crop_w)
                y1_min = max(0, int(np.floor(kp_y.min() - (crop_h - 1))))
                y1_max = min(int(np.floor(kp_y.max())), img_h - crop_h)

                # Check if there are valid ranges
                if x1_max >= x1_min and y1_max >= y1_min:
                    # Randomly select x1 and y1 from the valid ranges
                    x1 = np.random.randint(x1_min, x1_max + 1)
                    y1 = np.random.randint(y1_min, y1_max + 1)
                else:
                    # No valid crop that includes keypoints; perform a random crop
                    x1 = np.random.randint(0, img_w - crop_w + 1)
                    y1 = np.random.randint(0, img_h - crop_h + 1)
            else:
                # No visible keypoints; perform a random crop
                x1 = np.random.randint(0, img_w - crop_w + 1)
                y1 = np.random.randint(0, img_h - crop_h + 1)
        else:
            # No keypoints; perform a random crop
            x1 = np.random.randint(0, img_w - crop_w + 1)
            y1 = np.random.randint(0, img_h - crop_h + 1)
        ...
        # Ensure the crop area respects the crop size
        crop_y2 = min(crop_y2, img.shape[0])
        crop_x2 = min(crop_x2, img.shape[1])
        ...
        if results.get('gt_bboxes', None) is not None:
            ...
            # crop keypoints accordingly
            if results.get('gt_keypoints', None) is not None:
                keypoints = results['gt_keypoints']
                # Translate the keypoints based on the crop offsets
                keypoints[..., 0] -= crop_x1
                keypoints[..., 1] -= crop_y1

                # Clip keypoints to be within the image boundary
                valid_keypoints = (keypoints[..., 0] >= 0) & (keypoints[..., 0] < img_shape[1]) & \
                                (keypoints[..., 1] >= 0) & (keypoints[..., 1] < img_shape[0])

                # Update visibility 'v' (keypoints[..., 2]) to 0 for invalid keypoints
                keypoints[..., 2] = np.where(valid_keypoints, keypoints[..., 2], 0)
...
class RandomAffine(BaseTransform):
    ...
    def __init__(self,
        ...
        assert 0 <= prob <= 1, "Probability 'prob' should be between 0 and 1"
    ...
    @autocast_box_type()
    def transform(self, results: dict) -> dict:
        # Perform augmentation based on the probability p
        if random.random() > self.prob:
            # If the probability condition is not met, return the results unchanged
            return results
...
@TRANSFORMS.register_module()
class RandomAffineWithKeypoints(RandomAffine):
    """Random affine transform with keypoints support."""
    ...
    @autocast_box_type()
    def transform(self, results: dict) -> dict:
        # Perform augmentation based on the probability
        if random.random() > self.prob:
            return results
        ...
        center_x, center_y = width / 2, height / 2  # Image center coordinates

        # Get the affine transformation matrix (including rotation)
        warp_matrix = self._get_random_homography_matrix(height, width)

        # Modify the warp matrix to rotate around the image center
        # Step 1: Translate to the origin (image center)
        translation_to_center = np.array([[1, 0, -center_x], [0, 1, -center_y], [0, 0, 1]], dtype=np.float32)
        # Step 2: Apply rotation and other affine transformations
        # Step 3: Translate back to the original position
        translation_back = np.array([[1, 0, center_x], [0, 1, center_y], [0, 0, 1]], dtype=np.float32)

        # Combine the transformations: translate to center -> apply affine -> translate back
        warp_matrix = translation_back @ warp_matrix @ translation_to_center
        ...
        # Apply the transformation to keypoints
        keypoints = results.get('gt_keypoints', None)
        if keypoints is not None and num_bboxes:
            original_shape = keypoints.shape
            keypoints_flat = keypoints.reshape(-1, 3)

            # Create homogeneous coordinates by adding a column of ones to the keypoints
            keypoints_homogeneous = np.concatenate([keypoints_flat[:, :2], np.ones((keypoints_flat.shape[0], 1))], axis=1)

            # Apply the affine transformation using matrix multiplication
            transformed_keypoints = np.dot(warp_matrix, keypoints_homogeneous.T).T

            # Update the keypoints' x, y coordinates
            keypoints_flat[:, 0] = transformed_keypoints[:, 0]
            keypoints_flat[:, 1] = transformed_keypoints[:, 1]

            # Check if the transformed keypoints are still within the image boundaries
            valid_keypoints = (keypoints_flat[:, 0] >= 0) & (keypoints_flat[:, 0] < width) & \
                              (keypoints_flat[:, 1] >= 0) & (keypoints_flat[:, 1] < height)

            # Set visibility to 0 if the keypoint is outside the image boundaries
            keypoints_flat[:, 2] = np.where(valid_keypoints, keypoints_flat[:, 2], 0)

            # Reshape the keypoints back to the original shape
            keypoints = keypoints_flat.reshape(original_shape)
...
@TRANSFORMS.register_module()
class CachedMixUpWithKeypoints(CachedMixUp):
    """Cached mixup data augmentation with keypoints support."""
    ...
    def transform(self, results: dict) -> dict:
        ...
        retrieve_img = retrieve_results['img']
        with_mask = True if 'gt_masks' in results else False
        with_keypoints = 'gt_keypoints' in results  # Check if keypoints exist
        ...
        if with_keypoints:
            ...
            retrieve_gt_keypoints[..., 0] *= scale_ratio  # Scale x
            retrieve_gt_keypoints[..., 1] *= scale_ratio  # Scale y
        ...
        if is_flip:
            ...
            if with_mask:
                retrieve_gt_masks = retrieve_gt_masks.flip()
            if with_keypoints:
                retrieve_gt_keypoints[..., 0] = origin_w - retrieve_gt_keypoints[..., 0]
        ...
        if with_mask:
            ...
            retrieve_gt_masks = retrieve_gt_masks.translate(
                ...
                direction='vertical')
        if with_keypoints:
            retrieve_gt_keypoints[..., 0] -= x_offset
            retrieve_gt_keypoints[..., 1] -= y_offset
        ...
        if with_mask:
            mixup_gt_masks = retrieve_gt_masks.cat(
                [results['gt_masks'], retrieve_gt_masks])
        if with_keypoints:
            mixup_gt_keypoints = np.concatenate((results['gt_keypoints'], retrieve_gt_keypoints), axis=0)
...
@TRANSFORMS.register_module()
class CutMixWithKeypoints1(BaseTransform):
    """CutMix data augmentation with keypoints support."""
    ...
    @cache_randomness
    def _get_random_image(self) -> Union[dict, None]:
        """Select a random image from the cache that has valid annotations."""
        if not self.results_cache:
            return None

        for _ in range(15):  # max_iters
            index = random.randint(0, len(self.results_cache) - 1)
            cached_result = self.results_cache[index]
            if cached_result.get('gt_bboxes') is not None and len(cached_result['gt_bboxes']) > 0:
                return cached_result
        return None

    @autocast_box_type()
    def transform(self, results: dict) -> dict:
        results_org = copy.deepcopy(results)
        ...
        mix_results = self._get_random_image()
        if mix_results is None:
            return results

        # Original and mix images
        ori_img = results['img']
        ...
        if cut_w <= 0 or cut_h <= 0:
            return results  # Skip if patch dimensions are invalid
        ...
        ori_img[y1:y1 + cut_h, x1:x1 + cut_w] = mix_img[mix_y1:mix_y1 + cut_h, mix_x1:mix_x1 + cut_w]
        results['img'] = ori_img
        results['img_shape'] = ori_img.shape[:2]

        # Create patch_bbox as a tensor of shape [1, 4]
        x_min, y_min, x_max, y_max = x1, y1, x1 + cut_w, y1 + cut_h
        patch_bbox = results['gt_bboxes'].new_tensor([[x_min, y_min, x_max, y_max]])

        # Adjust bounding boxes and labels
        if 'gt_bboxes' in results and 'gt_bboxes_labels' in results:
            ori_bboxes = results['gt_bboxes']
            ori_labels = results['gt_bboxes_labels']
            mix_bboxes = mix_results['gt_bboxes']
            mix_labels = mix_results['gt_bboxes_labels']

            # Compute overlaps between ori_bboxes and patch_bbox
            overlaps = bbox_overlaps(ori_bboxes.tensor, patch_bbox)
            # Exclude ori_bboxes that overlap with the patch area
            keep_org_inds = overlaps.squeeze() == 0

            ori_bboxes = ori_bboxes[keep_org_inds]
            ori_labels = ori_labels[keep_org_inds]

            # Adjust mix_bboxes relative to the cut patch position
            mix_bboxes = mix_bboxes.clone()
            mix_bboxes.translate_([x1 - mix_x1, y1 - mix_y1])

            # Compute overlaps between mix_bboxes and patch_bbox
            overlaps = bbox_overlaps(mix_bboxes.tensor, patch_bbox)
            valid_mix_inds = overlaps.squeeze() > 0

            # Select valid bounding boxes and labels
            mix_bboxes = mix_bboxes[valid_mix_inds]
            mix_labels = mix_labels[valid_mix_inds]

    
            if len(ori_labels) == 0 and len(mix_labels) == 0:
                return results_org


            # Combine original and mix bounding boxes and labels
            combined_bboxes = type(ori_bboxes).cat([ori_bboxes, mix_bboxes])        
            combined_labels = np.concatenate([ori_labels, mix_labels], axis=0)
    
            results['gt_bboxes'] = combined_bboxes
            results['gt_bboxes_labels'] = combined_labels

        # Adjust keypoints
        if 'gt_keypoints' in results and 'gt_keypoints' in mix_results:
            ori_keypoints = results['gt_keypoints'][keep_org_inds, :, :]
            mix_keypoints = mix_results['gt_keypoints'][valid_mix_inds, :, :]

            # Adjust mix_keypoints relative to the cut patch position
            mix_keypoints = mix_keypoints.copy()
            mix_keypoints[..., 0] += x1 - mix_x1
            mix_keypoints[..., 1] += y1 - mix_y1

            # Determine which keypoints are inside the patch
            x_min, y_min, x_max, y_max = x1, y1, x1 + cut_w, y1 + cut_h
            valid_kp = (mix_keypoints[..., 0] >= x_min) & (mix_keypoints[..., 0] <= x_max) & \
                       (mix_keypoints[..., 1] >= y_min) & (mix_keypoints[..., 1] <= y_max)
            mix_keypoints[..., 2] = np.where(valid_kp, mix_keypoints[..., 2], 0)

            # Combine keypoints
            combined_keypoints = np.concatenate([ori_keypoints, mix_keypoints], axis=0)
            results['gt_keypoints'] = combined_keypoints

        # Adjust masks if present
        if 'gt_masks' in results and 'gt_masks' in mix_results:
            ori_masks = results['gt_masks'][keep_org_inds]
            mix_masks = mix_results['gt_masks'][valid_mix_inds]

            # Adjust mix_masks relative to the cut patch position
            mix_masks = mix_masks.copy()
            mix_masks.translate_([x1 - mix_x1, y1 - mix_y1])

            # Combine masks
            combined_masks = ori_masks.cat([ori_masks, mix_masks])
            results['gt_masks'] = combined_masks

        # Update ignore flags and instance IDs if present
        if 'gt_ignore_flags' in results and 'gt_ignore_flags' in mix_results:
            ori_ignore_flags = results['gt_ignore_flags'][keep_org_inds]
            mix_ignore_flags = mix_results['gt_ignore_flags'][valid_mix_inds]

            combined_ignore_flags = np.concatenate([ori_ignore_flags, mix_ignore_flags], axis=0)
            results['gt_ignore_flags'] = combined_ignore_flags

        if 'gt_instances_ids' in results and 'gt_instances_ids' in mix_results:
            ori_ids = results['gt_instances_ids'][keep_org_inds]
            mix_ids = mix_results['gt_instances_ids'][valid_mix_inds]

            combined_ids = np.concatenate([ori_ids, mix_ids], axis=0)
            results['gt_instances_ids'] = combined_ids

        # Optionally adjust segmentation maps
        if 'gt_seg_map' in results and 'gt_seg_map' in mix_results:
            ori_seg = results['gt_seg_map'][keep_org_inds]
            mix_seg = mix_results['gt_seg_map'][valid_mix_inds]

            ori_seg = ori_seg.copy()
            ori_seg[y1:y1 + cut_h, x1:x1 + cut_w] = mix_seg[mix_y1:mix_y1 + cut_h, mix_x1:mix_x1 + cut_w]
            results['gt_seg_map'] = ori_seg
...
@TRANSFORMS.register_module()
class CutMixWithKeypoints(BaseTransform):
    """CutMix data augmentation with keypoints support."""
    ...
    def transform(self, results: dict) -> dict:
        ...
        # Create patch_bbox as a tensor of shape [1, 4]
        device = results['gt_bboxes'].tensor.device if hasattr(results['gt_bboxes'], 'tensor') else 'cpu'
        patch_bbox = results['gt_bboxes'].new_tensor([[x1, y1, x2, y2]], device=device)

        # Adjust bounding boxes and labels
        if 'gt_bboxes' in results and 'gt_bboxes_labels' in results:
            ori_bboxes = results['gt_bboxes']
            ori_labels = np.array(results['gt_bboxes_labels'])

            # Exclude original bounding boxes that overlap with the patch or have any keypoints within the patch
            if len(ori_bboxes) > 0:
                # Compute overlaps between ori_bboxes and patch_bbox
                overlaps = bbox_overlaps(ori_bboxes.tensor, patch_bbox)
                overlaps = overlaps.cpu().numpy()
                # Initialize mask for bounding boxes to keep
                keep_inds = overlaps[:, 0] == 0  # Keep boxes with no overlap

                # Exclude objects if any keypoint is inside the patch
                if 'gt_keypoints' in results:
                    ori_keypoints = results['gt_keypoints']
                    # For each object, check if any keypoint is inside the patch
                    x_min, y_min, x_max, y_max = x1, y1, x2, y2
                    kp_in_patch = ((ori_keypoints[..., 0] >= x_min) & (ori_keypoints[..., 0] <= x_max) &
                                (ori_keypoints[..., 1] >= y_min) & (ori_keypoints[..., 1] <= y_max))
                    kp_visible = ori_keypoints[..., 2] > 0
                    kp_overlap = np.any(kp_in_patch & kp_visible, axis=1)
                    # Exclude objects with any keypoint inside the patch
                    keep_inds = keep_inds & (~kp_overlap)

                # Filter annotations based on keep_inds
                ori_bboxes = ori_bboxes[keep_inds]
                ori_labels = ori_labels[keep_inds]
                if 'gt_keypoints' in results:
                    ori_keypoints = ori_keypoints[keep_inds]
                if 'gt_ignore_flags' in results:
                    ori_ignore_flags = results['gt_ignore_flags'][keep_inds]
                if 'gt_instances_ids' in results:
                    ori_ids = results['gt_instances_ids'][keep_inds]
                if 'gt_masks' in results:
                    ori_masks = results['gt_masks'][keep_inds]
            else:
                # No bounding boxes in original image
                ori_bboxes = type(results['gt_bboxes'])(torch.empty((0, 4), device=device))
                ori_labels = np.empty((0,), dtype=np.int64)
                if 'gt_keypoints' in results:
                    ori_keypoints = np.empty((0, 3, 3), dtype=np.float32)
                if 'gt_ignore_flags' in results:
                    ori_ignore_flags = np.empty((0,), dtype=bool)
                if 'gt_instances_ids' in results:
                    ori_ids = results['gt_instances_ids'][:0]
                if 'gt_masks' in results:
                    ori_masks = results['gt_masks'][:0]

            # Adjust mix bounding boxes and labels
            mix_bboxes = mix_results['gt_bboxes']
            mix_labels = np.array(mix_results['gt_bboxes_labels'])
            ...
            if len(mix_bboxes) > 0:
                ...
                # Clip mix_bboxes to the image boundaries
                mix_bboxes.clip_([h, w])

                # Compute overlaps between mix_bboxes and patch_bbox (should be fully inside)
                overlaps = bbox_overlaps(mix_bboxes.tensor, patch_bbox)
                overlaps = overlaps.cpu().numpy()
                valid_inds = overlaps[:, 0] > 0

                if 'gt_keypoints' in mix_results:
                    mix_keypoints = mix_results['gt_keypoints']
                    mix_keypoints = mix_keypoints[valid_inds]
                    mix_keypoints = mix_keypoints.copy()
                    mix_keypoints[..., 0] += x1 - mix_x1
                    mix_keypoints[..., 1] += y1 - mix_y1
                ...
                mix_bboxes = mix_bboxes[valid_inds]
                mix_labels = mix_labels[valid_inds]
                if 'gt_ignore_flags' in mix_results:
                    mix_ignore_flags = mix_results['gt_ignore_flags'][valid_inds]
                if 'gt_instances_ids' in mix_results:
                    mix_ids = mix_results['gt_instances_ids'][valid_inds]
                if 'gt_masks' in mix_results:
                    mix_masks = mix_results['gt_masks'][valid_inds]
                    mix_masks = mix_masks.copy()
                    mix_masks.translate_([x1 - mix_x1, y1 - mix_y1])
            else:
                # No bounding boxes in mix image
                mix_bboxes = type(results['gt_bboxes'])(torch.empty((0, 4), device=device))
                mix_labels = np.empty((0,), dtype=np.int64)
                if 'gt_keypoints' in mix_results:
                    mix_keypoints = np.empty((0, 3, 3), dtype=np.float32)
                if 'gt_ignore_flags' in mix_results:
                    mix_ignore_flags = np.empty((0,), dtype=bool)
                if 'gt_instances_ids' in mix_results:
                    mix_ids = mix_results['gt_instances_ids'][:0]
                if 'gt_masks' in mix_results:
                    mix_masks = mix_results['gt_masks'][:0]

            # If no annotations are left after filtering, return the original results
            if len(ori_labels) == 0 and len(mix_labels) == 0:
                return results_org

            # Combine annotations from original and mix images
            combined_bboxes = ori_bboxes.cat([ori_bboxes, mix_bboxes])
            combined_labels = np.concatenate([ori_labels, mix_labels], axis=0)
            results['gt_bboxes'] = combined_bboxes
            results['gt_bboxes_labels'] = combined_labels

            if 'gt_keypoints' in results and mix_keypoints is not None:
                combined_keypoints = np.concatenate([ori_keypoints, mix_keypoints], axis=0)
                results['gt_keypoints'] = combined_keypoints
            elif 'gt_keypoints' in results:
                results['gt_keypoints'] = ori_keypoints
            elif mix_keypoints is not None:
                results['gt_keypoints'] = mix_keypoints
        ...
        # Optionally adjust segmentation maps
        if 'gt_seg_map' in results and 'gt_seg_map' in mix_results:
            ori_seg = results['gt_seg_map']
            mix_seg = mix_results['gt_seg_map']
            ori_seg = ori_seg.copy()
            ori_seg[y1:y2, x1:x2] = mix_seg[mix_y1:mix_y2, mix_x1:mix_x2]
            results['gt_seg_map'] = ori_seg
...
@TRANSFORMS.register_module()
class RandomResizeWithKeypoints(BaseTransform):
    """Randomly resize images and corresponding annotations like bounding boxes
    and keypoints.

    The target scale for resizing is randomly selected based on the provided
    `scale` and `ratio_range` parameters.
    ...
    Modified Keys:
        - img
        - img_shape
        - gt_bboxes (if present)
        - gt_keypoints (if present)
    ...
    Args:
        scale (tuple or Sequence[tuple]): Images scales for resizing.
            If a tuple of int, it should be (width, height). If a sequence of
            tuples, it should contain two tuples specifying the lower and upper
            bounds of image scales.
        ratio_range (tuple[float], optional): (min_ratio, max_ratio).
            Used to randomly scale the image by a ratio. Defaults to None.
        keep_ratio (bool, optional): Whether to keep the aspect ratio when
            resizing. Defaults to True.
        backend (str, optional): Image resize backend. Defaults to 'cv2'.
        interpolation (str, optional): Interpolation method. Defaults to 'bilinear'.

    Note:
        - If `scale` is a sequence of tuples, a random scale will be sampled
          between the provided scales.
        - If `ratio_range` is specified, a random ratio will be sampled and
          multiplied with `scale` to get the target scale.
    """
    ...
    def __init__(
        ...
        # Placeholder for the resize transform
        self.resize = None

    @staticmethod
    def _random_sample(scales: Sequence[Tuple[int, int]]) -> Tuple[int, int]:
        """Randomly sample a scale from a list of scales.

        Args:
            scales (Sequence[tuple]): Two tuples specifying the lower and upper
                bounds of image scales.

        Returns:
            tuple: The randomly sampled scale.
        """
        assert mmengine.is_seq_of(scales, tuple) and len(scales) == 2, \
            "When 'scale' is a sequence, it must contain two tuples."
        scale_0_w, scale_1_w = scales[0][0], scales[1][0]
        scale_0_h, scale_1_h = scales[0][1], scales[1][1]
        width = np.random.randint(min(scale_0_w, scale_1_w), max(scale_0_w, scale_1_w) + 1)
        height = np.random.randint(min(scale_0_h, scale_1_h), max(scale_0_h, scale_1_h) + 1)
        return (width, height)

    @staticmethod
    def _random_sample_ratio(scale: Tuple[int, int], ratio_range: Tuple[float, float]) -> Tuple[int, int]:
        """Randomly sample a scale by applying a random ratio to the base scale.

        Args:
            scale (tuple): The base scale (width, height).
            ratio_range (tuple[float]): The range of ratios to sample from.
        ...
        width = int(scale[0] * ratio + 0.5)
        height = int(scale[1] * ratio + 0.5)
        return (width, height)

    def _random_scale(self) -> Tuple[int, int]:
        """Randomly sample a target scale based on the initialization parameters.

        Returns:
            tuple: The target scale (width, height).
        """
        if mmengine.is_tuple_of(self.scale, int):
            assert self.ratio_range is not None and len(self.ratio_range) == 2, \
                "'ratio_range' must be specified when 'scale' is a tuple of int."
            ...
            raise ValueError(f"Invalid 'scale' argument: {self.scale}")
        return scale

    @autocast_box_type()
    def transform(self, results: dict) -> dict:
        """Apply the random resize transformation to the image and annotations.

        Args:
            results (dict): The dictionary containing the image and annotations.

        Returns:
            dict: The transformed results with updated keys.
        """
        ...
        if self.keep_ratio:
            scale_factor = min(scale[0] / orig_shape[1], scale[1] / orig_shape[0])
            new_size = (int(orig_shape[1] * scale_factor + 0.5), int(orig_shape[0] * scale_factor + 0.5))
            scale_factor = np.array([scale_factor, scale_factor], dtype=np.float32)
        else:
            new_size = scale
            scale_factor_w = scale[0] / orig_shape[1]
            scale_factor_h = scale[1] / orig_shape[0]
            scale_factor = np.array([scale_factor_w, scale_factor_h], dtype=np.float32)
        ...
        # Resize annotations if present
        if 'gt_bboxes' in results:
            bboxes = results['gt_bboxes'].tensor.float()
            bboxes[:, [0, 2]] *= scale_factor[0]
            bboxes[:, [1, 3]] *= scale_factor[1]
            results['gt_bboxes'] = type(results['gt_bboxes'])(bboxes)

        if 'gt_keypoints' in results:
            keypoints = results['gt_keypoints']
            keypoints[..., :2] = keypoints[..., :2] * scale_factor[np.newaxis, np.newaxis, :]
            results['gt_keypoints'] = keypoints

        if 'gt_seg_map' in results:
            seg_map = results['gt_seg_map']
            seg_map = mmengine.imresize(
                seg_map,
                new_size,
                interpolation='nearest',
                backend=self.backend
...
@TRANSFORMS.register_module()
class HandColorAugmentation(BaseTransform):
