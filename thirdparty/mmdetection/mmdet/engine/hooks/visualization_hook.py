...
def build_tar_index(tar_path):
    ...
    with tarfile.open(tar_path, 'r') as tar:
        for tarinfo in tqdm(tar, desc='building tar index'):
            index[tarinfo.name] = (tarinfo.offset_data, tarinfo.size)
    return open(tar_path, 'rb'), index
...
class DetVisualizationHook(Hook):
    ...
    def __init__(self,
        ...
        self.tar_file_cache = dict()

    def get_image_bytes(self, img_path):
        path_split = img_path.split(os.path.sep)
        
        if path_split[-2].endswith('.tar'):
            tar_path = f'{os.path.sep}'.join(path_split[:-1])
            sample_name = path_split[-1]

            if tar_path not in self.tar_file_cache: 
                self.tar_file_cache[tar_path] = build_tar_index(tar_path)

            tar_file, tar_index = self.tar_file_cache[tar_path]
            tar_offset, file_size = tar_index[sample_name]
            tar_file.seek(tar_offset)
            img_bytes = tar_file.read(file_size)
        else:
    ...
    def after_val_iter(self, runner: Runner, batch_idx: int, data_batch: dict,
        ...
        img_bytes = self.get_image_bytes(img_path)
