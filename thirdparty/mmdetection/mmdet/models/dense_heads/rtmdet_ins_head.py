...
from mmdet.models.nms import keypoint_nms, wrist_keypoint_nms
...
class RTMDetInsHead(RTMDetHead):
    ...
    def _bbox_mask_post_process(
        ...
        img_h, img_w = img_meta['ori_shape'][:2] if rescale else img_meta[
            'img_shape'][:2]
        ...
        if results.bboxes.numel() > 0:
            ...
            # cfg.nms.update({'class_agnostic': False})
            # cfg.nms.update({'iou_threshold': 0.1})

            # print(cfg.nms)
            det_bboxes, keep_idxs = batched_nms(bboxes.to(torch.float32), results.scores.to(torch.float32),
                                                results.labels.to(torch.float32), cfg.nms)
            results = results[keep_idxs]
            # some nms would reweight the score, such as softnms
            results.scores = det_bboxes[:, -1]
            results = results[:cfg.max_per_img]

            results.keypoints = torch.zeros(
                size=(results.bboxes.shape[0], self.n_kpts, 2),
                dtype=torch.bool,
            ...
            kpt_logits = self._mask_predict_by_feat_single(
                mask_feat, results.kernels, results.priors)

            kpt_logits = kpt_logits[..., :2] * stride

            if rescale:
                # print(img_meta['scale_factor'])

                kpt_logits[..., 0] = kpt_logits[..., 0] / img_meta['scale_factor'][0]
                kpt_logits[..., 1] = kpt_logits[..., 1] / img_meta['scale_factor'][1]   

                # exit(0)
                # ori_h, ori_w = img_meta['ori_shape'][:2]
                
                # print(kpt_logits.shape, stride, rescale)
                # exit(0)
        ...
        results = wrist_keypoint_nms(results, img_h, img_w)
        ...
        for i in range(n_layers):
            if i < n_layers - 1:
                weight_splits[i] = weight_splits[i].reshape(
                    self.n_kpts * n_inst * self.dyconv_channels, -1, 1, 1)
                bias_splits[i] = bias_splits[i].reshape(self.n_kpts * n_inst * self.dyconv_channels)
            else:
                weight_splits[i] = weight_splits[i].reshape(self.n_kpts * n_inst, -1, 1, 1)
                bias_splits[i] = bias_splits[i].reshape(self.n_kpts * n_inst)
    ...
    def _mask_predict_by_feat_single(self, mask_feat: Tensor, kernels: Tensor,
        ...
        relative_coord = relative_coord.repeat(self.n_kpts, 1, 1, 1)
        
        mask_feat = torch.cat(
            [relative_coord,
             mask_feat.repeat(self.n_kpts * num_inst, 1, 1, 1)], dim=1)
        ...
        for i, (weight, bias) in enumerate(zip(weights, biases)):
            x = F.conv2d(
                x, weight, bias=bias, stride=1, padding=0, groups=self.n_kpts * num_inst)
            if i < n_layers - 1:
                x = F.relu(x)
                
        x = x.reshape(num_inst, self.n_kpts, h, w)
        x = self.integral_pose_regression(x)
    ...
    def loss_mask_by_feat(self, mask_feats: Tensor, flatten_kernels: Tensor,
        ...
        for idx, (mask_feat, kernels, sampling_results,
            ...
            pos_kpt_logits = self._mask_predict_by_feat_single(
                mask_feat, pos_kernels, pos_priors)
            
            # import cv2; import numpy as np
            # i = idx
            # img = cv2.imread(f'/cmillerd/Projects/Aria/arm_detection/mmdet_test/imgs/img_{i}.jpg')  
            # print(gt_instances.keypoints.shape, gt_instances.bboxes.shape, img.shape)
            # for bbox in gt_instances.bboxes.cpu().numpy():
            #     x1, y1, x2, y2 = bbox
            #     cv2.rectangle(img, (int(x1), int(y1)), (int(x2), int(y2)), (0, 255, 0), 2)
            # for x, y, _ in gt_instances.keypoints.view(-1, 3).cpu().numpy():
            #     cv2.circle(img, (int(x), int(y)), 2, (255, 0, 0), -1)
            # cv2.imwrite(f'/cmillerd/Projects/Aria/arm_detection/mmdet_test/imgs/img_{i}.jpg', img)


            if gt_instances.keypoints.numel() == 0:
                gt_keypoints = torch.empty_like(gt_instances.keypoints)
            else:
            ...
            pos_pred_keypoints.append(pos_kpt_logits)
            pos_gt_keypoints.append(gt_keypoints)

        pos_pred_keypoints = torch.cat(pos_pred_keypoints, 0)
        pos_gt_keypoints = torch.cat(pos_gt_keypoints, 0)
        
        if pos_pred_keypoints.shape[0] == 0:
            return mask_feats.sum() * 0

        scale = self.prior_generator.strides[0][0] #// self.mask_loss_stride
        # scale keypoint coordinates 
        pos_pred_keypoints = pos_pred_keypoints[..., :2] * scale

        valid_gt_kpts = pos_gt_keypoints[..., 2] > 0
        pos_gt_keypoints = pos_gt_keypoints[..., :2]

        valid_gt_kpts = valid_gt_kpts.view(-1)
        pos_pred_keypoints = pos_pred_keypoints.view(-1, 2)
        pos_gt_keypoints = pos_gt_keypoints.view(-1, 2)

        pos_pred_keypoints = pos_pred_keypoints[valid_gt_kpts]
        pos_gt_keypoints = pos_gt_keypoints[valid_gt_kpts]
        ...
        # avg_factor
        num_pos = pos_pred_keypoints.shape[0]
...
class RTMDetInsSepBNHead(RTMDetInsHead):
    ...
    def _init_layers(self) -> None:
        ...
        for i in range(self.num_dyconvs):
            if i == 0:
                weight_nums.append(
                    (self.num_prototypes + 2) * self.dyconv_channels * self.n_kpts)
                bias_nums.append(self.dyconv_channels * self.n_kpts)
            elif i == self.num_dyconvs - 1:
                weight_nums.append(self.dyconv_channels * self.n_kpts)
                bias_nums.append(1 * self.n_kpts)
            else:
                weight_nums.append(self.dyconv_channels * self.dyconv_channels * self.n_kpts)
                bias_nums.append(self.dyconv_channels * self.n_kpts)
