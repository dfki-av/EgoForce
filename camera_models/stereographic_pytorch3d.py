...
class StereographicCameraPytorch3D(CamerasBase):
	_FIELDS = (
		"focal_length",
		"principal_point",
		"R",
	...
	def evaluate(self, points_cam: torch.Tensor) -> torch.Tensor:
		...
		eps = 1e-12
		denom = r + z
		denom_safe = torch.where(denom >= 0, torch.clamp(denom, min=eps), torch.clamp(denom, max=-eps))
	...
	def unproject_points(
		...
		**kwargs,
		...
		assert scaled_depth_input, "scaled_depth_input must be True for StereographicCameraPytorch3D"
