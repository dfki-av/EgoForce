...
class EquisolidCameraPytorch3D(CamerasBase):
	_FIELDS = (
		"focal_length",
		"principal_point",
		"R",
	...
	def evaluate(self, points_cam: torch.Tensor) -> torch.Tensor:
		...
		eps = 1e-12
		rxy = torch.linalg.norm(points_cam[..., :2], dim=-1, keepdim=True)
	...
	def inverse_evaluate(self, q: torch.Tensor) -> torch.Tensor:
		...
		rays = rays / torch.clamp(torch.linalg.norm(rays, dim=-1, keepdim=True), min=eps)
		return rays
	...
	def unproject_points(
		...
		**kwargs,
		...
		assert scaled_depth_input, "scaled_depth_input must be True for EquisolidCameraPytorch3D"
